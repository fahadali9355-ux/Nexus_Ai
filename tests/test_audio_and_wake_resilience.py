...
def test_fix_c_wake_word_resilience():
    print("=" * 65)
    print("  TESTING FIX C: VOSK WAKE WORD TIMEOUT & RESILIENCE              ")
    print("=" * 65)

    # 1. Test timeout handling
    print("[Fix C Test 1] Testing listen_for_wake_word with 2.0s timeout...")
    start_t = time.time()
    result = listen_for_wake_word(wake_word="nexus", listen_timeout=2.0)
    elapsed = time.time() - start_t
    ...
    # 2. Test invalid model path handling
    print("\n[Fix C Test 2] Testing listen_for_wake_word with invalid custom model path...")
    safe_result = listen_for_wake_word(model_path="nonexistent_invalid_model_dir_12345", listen_timeout=1.0)
    print(f"[Fix C Result 2] Safe return on invalid model path: {safe_result}")
    assert safe_result is False, "Listener should return False gracefully on missing model"
